...
<?php
	...
	$temId   = $sHandler->getSurveyLinkID();
	$temName = $sHandler->getSurveyLinkName();
...
$(document).ready(function(){
	...
	var temName = <?php echo json_encode($temName); ?>;
	var temId  = <?php echo json_encode($temId); ?>;

	$('#gallery'+temId).touchSlider({
		mode: 'index',
		prevLink: 'a.prev',
		nextLink: 'a.next',
		onChange: function(prev, curr) {
			$('#counter'+temId).html((curr+1)+'/'+ $('#gallery'+temId).get(0).getCount());
		},
		onStart: function(){
			$('#counter'+temId).html('1/' + $('#gallery'+temId).get(0).getCount());
		}
	});
...
<!--Template-->
<?php echo "<div data-role='page' name='tempSurvey' id='". $temId ."'>"; ?>
			<div data-role='header' data-position='fixed' class='header'>
	    			<a href='user.php#templateSurvey' rel='external' class='backBtn' data-corners='false' data-shadow='false' data-role='button' data-transition='pop' data-direction='reverse'>Back</a>
	    		
	    	<?php	echo "<h1>" . $temName . "</h1>"; ?>
	    		<a  href='#confirmDia' class='ui-btn-right' data-role='button' data-transition='pop' data-rel='popup' data-corners='false' data-shadow='false' style='background:#c0362c;'>Select</a>
				</div>
				<div data-role='popup' id='confirmDia' style='width:250px;color:black;background:rgba(255,255,255,0.9);text-align:center;margin-top:30%;margin-right:18px;'> 
					...
					<div data-role='content'>
						<h3 style='margin-bottom:10%;'>Confirm selection ?</h3>
						<a id='confirmBtn' herf='' data-role='button' data-inline='true' data-corners='false' data-mini='true' data-shadow='false' style='width:65px;margin-right:25px;'>Yes</a>
				...
				<script type="text/javascript">
					$(document).ready(function(){
						$("#confirmBtn").click(function () {
							var survey_name = <?php echo json_encode($temName); ?>;
							sendInfo(survey_name);
						});
						function sendInfo(survey_name){
							$.ajax ({
								url: "php/addTemSurvey.php",
								type:"POST",
								data:{temSurvey_name:survey_name},
				...
				<div data-role='content' class='survey' >
					<form action='' method='post' id='' data-ajax='false'>
						<div >
							<?php echo "<h3 style='text-align:center;text-shadow:none;color:black'>" . $temName . "</h3>"; ?>
						</div>
					<div class='gallery_holder'>
<?php					
					echo "<div class='gallery' name='tempSurvey'  id='gallery" . $temId . "'>
							<div class='holder'>
								<div class='list'>";
									$qHandler = new QuestionHandler;
									if($qHandler->getQuestionsBySurveyName($temName)){
									...
									<div data-role='fieldcontain' class='direction' >
										...
										<span class='counter' name='tempSurvey' id='counter" . $temId . "'></span>
									</div>
								</div> <!--end gallery-->
